<template>
  <div class="login">
    ...
    <div class="login-container">
      <p><input placeholder="Name" v-model="name"></p>
      <p><input type="password" placeholder="Password" v-model="password"></p>
      <p><input type="submit" value="Log in" @click="login(name, password)"></p>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const login = (name: string, password: string) => {
    ...
    .then((res: any) => {
      if (res.data.login.Message.includes("Hello")) {
        // アップロード画面へ遷移
        router.push("/upload");
      } else {
        window.alert("名前またはパスワードが間違っています");
      }
    });
  }
</script>
